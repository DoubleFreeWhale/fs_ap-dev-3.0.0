<template>
  <ifbp-page class="ifbp-statements">
    <!-- 左侧树节点 -->
...
export default {
  data() {
    ...
    return {
      ...
      showTree: true,
      treeStructure:{},
      funcode: window.vueInstance.$route.query.funcode,
      isMultiPage:true
    };
  },
  watch:{
    $route(val, oldVal) {
      if(val.path=='/fbpdap/statements'){
        this.funcode = window.vueInstance.$route.query.funcode;
        this.getTreeData();
      }
    },
    deep: true,
  ...
  mounted() {
    ...
    window.onresize = () => {
      this.$nextTick(()=>{
        $('.ifbp-statements .right-container').height(window.innerHeight-$('header.header').height());
      });
    }
  },
  methods: {
    ...
    getTreeData() {
      // ajax 获取树数据
      const requestUrl = '/dap/reportbrowse/getreports?type=func&code='+this.funcode;
    ...
    getUrl(reportKey,nodeName){
      // ajax 获取 iframeUrl
      this.iframeUrl = null;
      const requestUrl = '/dap/reportbrowse/geturl?type=func&code='+ this.funcode+'&pk_report='+reportKey;
      this.$http.get(requestUrl).then(res => {
        if (res.data.status) {
...
<style scoped>
  ...
  .ifbp-statements .flex-btn:hover{
    background: #EAF9FF;
  }
  .ifbp-statements .flex-btn i{
    font-size:16px;
    color:#8E8E93;
    vertical-align: -moz-middle-with-baseline;
  }
  .ifbp-statements.main-panel{
    padding:0;
  }
</style>