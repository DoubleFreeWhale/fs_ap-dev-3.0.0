<template>
    <div id="editor" v-bind:class="['boxscroll',isNewtem ?'isNewtem':'']">
...
<script>
    ...
    import directives from "../js/editor/directives";
    import elToolsPanel from './elToolsPanel'
    ...
    export default {
        ...
        methods: {
            doGetModel: function () {
                compUtil.removeDesignAttr(this.currentDom);
                this.formInfo.pkMetadata = this.pkMetadata;
                var model = {
                    form: this.formInfo,
                    formLayout: this.currentDom.html(),
                    formData: this.tplData,
                    formMethods:this.tplMethods,
                    formVersion: '1'
                }
                compUtil.repairAttr(this.currentDom);
                return model;
            },
            doSave: function (isEnable) {
                var oThis = this;
                var model = this.getModel(isEnable);
                var param = {
                    data: JSON.stringify(model),
                    pkMetadata: this.pkMetadata,
                    pk_temp: parent.nowPkTemp || model.form.pk_temp
                }
                // 用于康复刷数据用的
                        ...
                        if (data && data.message) {
                            ...
                            oThis.$message({
                                ...
                                onClose: function () {
                                    ...
                                    //     data = JSON.parse(data);
                                    // }
                                    var pk_temp = data.form.pk_temp;
                                    if (pk_temp && parent.nowPkTemp !== pk_temp) {
                                        var str = '';
                                        if(parent.shell.isNewtem){
                                            str = '&isNewtem=true';
                                        }
                                        parent.location.href = parent.location.origin + '/uitemplate_web/index.html?pk_temp=' + pk_temp + str;
                                    }
                                }
                            });
                        }
            ...
            saveWithFeedback: function(isEnable) {
                var oThis = this;
                var model = this.getModel(isEnable);
                var param = {
                    data: JSON.stringify(model),
                ...
                $.ajax({
                    url: "/uitemplate_web/uitemplate_ctr/uitemplate_design_ctr/saveTemplateDP",
                    type: 'POST',
                    data: param,
                    success: function (data) {
                        if(top.vueInstance && top.vueInstance.$children[0] && top.vueInstance.$children[0].$eventBus){
                            if (data.success) {
                                top.vueInstance.$children[0].$eventBus.$emit('uitemplateSave');
                            } else if (data.error && data.error.errorMessage) {
                                top.vueInstance.$children[0].$eventBus.$emit('uitemplateSaveError', data.error.errorMessage);
                            } else {
                                top.vueInstance.$children[0].$eventBus.$emit('uitemplateSaveError', '未知错误, 未保存成功...');
                            }
                        }
                    }
                });
            },
            doLoadModel: function (model) {
                this.tplData = $.extend(this.tplData, model.formData); //model.formData || this.tplData;
                this.formInfo = model.form || {
                    name: '标题'
                };
                var formLayout = model.formLayout || "";
                if (formLayout && formLayout.layoutType == 'pc' || formLayout.layoutDetail) { //兼容旧版本UI模板
                    var $dom = compUtil._getDefaultCompDom('el-form');
                    this.currentDom.append($dom);
                    if (typeof formLayout.layoutDetail == 'string') {
                        formLayout.layoutDetail = JSON.parse(formLayout.layoutDetail);
                    }
                    if (parent.shell.isIform) {
                        this.iformAssemDom(formLayout.layoutDetail, $dom);
                    } else {
                        this.assemDom(formLayout.layoutDetail, $dom);
                    }

                } else {
                    var $dom = $(formLayout);
                    if (!$dom || ($dom.length < 1)) {} else {
                        // 兼容之前的处理
                        if ($dom[0].localName === 'el-form') {
                            var $targetDom = $(this.currentDom).find('#form_area');
                        } else if ($dom[0].localName === 'el-table') {
                            var $targetDom = $(this.currentDom).find('#table_area');
                        } else {
                            // 新的模板处理
                            var $targetDom = $(this.currentDom);
                        }
                        $targetDom.empty().append($dom);
                        this.resetDomAttr($targetDom);
        ...
        mounted() {
                ...
                console.log(window.top.shell);
            window.top.engine=this
        }
    }
</script>
