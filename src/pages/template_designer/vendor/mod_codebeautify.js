(function() {
    ...
    function b(g, F) {
        ...
        function f(aw) {
            var at = false, au = "", az = 0, av = "", ax = 0, ay;
...
(function() {
    ...
    b.prototype.xml = function(l, f) {
        var h = l.replace(/>\s{0,}</g, "><").replace(/</g, "~::~<").replace(/\s*xmlns\:/g, "~::~xmlns:").replace(/\s*xmlns\=/g, "~::~xmlns=").split("~::~")
          , i = h.length
          , m = false
          , k = 0
          , j = ""
    ...
    b.prototype.css = function(l, j) {
        var i = l.replace(/\s{1,}/g, " ").replace(/\{/g, "{~::~").replace(/\}/g, "~::~}~::~").replace(/\;/g, ";~::~").replace(/\/\*/g, "~::~/*").replace(/\*\//g, "*/~::~").replace(/~::~\s{0,}~::~/g, "~::~").split("~::~")
          , e = i.length
          , h = 0
          , k = ""
          , g = 0
    ...
    b.prototype.sql = function(q, g) {
        var n = q.replace(/\s{1,}/g, " ").replace(/\'/ig, "~::~'").split("~::~")
          , l = n.length
          , j = []
          , p = 0
          , h = this.step
...
(function() {
    function a(v, e) {
        ...
        function y() {
            var z = j;
            do {} while (h.test(r()));return j !== z + 1
        }
        function q() {
            var z = j;
            r();
    ...
    function b(v, g, p, h) {
        ...
        function f() {
            ...
            this.get_tag = function(J) {
                var E = "", G = [], F = "", A = false, I, D, B = this.pos, t = this.line_char_count;
            ...
            this.get_unformatted = function(A, B) {
                ...
                do {
                    ...
                    this.line_char_count++;
                    D = true
                } while (C.toLowerCase().indexOf(A) === -1);return C
            }
            ;
...
//     var a = function() {
//         if (b == "Javascript") {
//             var i = js_beautify($("#codeSource").val(), c);
//             i = i.replace(/>/g, "&gt;").replace(/</g, "&lt;");
//             i = '<pre class="brush: js;toolbar:false;">' + i + "</pre>";
//             $("#jfContent").html(i)
//         } else {
//             if (b == "CSS") {
//                 var h = css_beautify($("#codeSource").val());
//                 h = h.replace(/>/g, "&gt;").replace(/</g, "&lt;");
//                 h = '<pre class="brush: css;toolbar:true;">' + h + "</pre>";
//                 $("#jfContent").html(h)
//             } else {
//                 if (b == "HTML") {
//                     var g = html_beautify($("#codeSource").val());
//                     g = g.replace(/>/g, "&gt;").replace(/</g, "&lt;");
//                     g = '<pre class="brush: html;toolbar:false;">' + g + "</pre>";
//                     $("#jfContent").html(g)
//                 } else {
//                     if (b == "XML") {
//                         var f = vkbeautify.xml($("#codeSource").val());
//                         f = f.replace(/>/g, "&gt;").replace(/</g, "&lt;");
//                         f = '<pre class="brush: html;toolbar:false;">' + f + "</pre>";
//                         $("#jfContent").html(f)
//                     } else {
//                         if (b == "SQL") {
//                             var j = vkbeautify.sql($("#codeSource").val(), 4);
//                             j = j.replace(/>/g, "&gt;").replace(/</g, "&lt;");
//                             j = '<pre class="brush: sql;toolbar:false;">' + j + "</pre>";
//                             $("#jfContent").html(j)
//                         }
//                     }
//                 }
...
//             if (g.type == MSG_TYPE.TAB_CREATED_OR_UPDATED && g.event == "codebeautify") {
//                 if (g.content) {
//                     document.getElementById("codeSource").value = (g.content);
//                     a()
//                 }
//             }
//         });
