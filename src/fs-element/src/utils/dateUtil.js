...
var dateUtil =  {
  ...
  _formats: {
      //year
      YY   : function (date) {
          return dateUtil.leftZeroFill(date.getFullYear() % 100, 2);
      },
      YYYY : function (date) {
      ...
      MM: function(date){
          var m = dateUtil._formats.M(date);
          return dateUtil.leftZeroFill(m,2);
      },
      MMM  : function (date, language) {
          var m = date.getMonth();
          return dateUtil._dateLocale[language].monthsShort[m];
      },
      MMMM : function (date, language) {
          var m = date.getMonth();
          return dateUtil._dateLocale[language].months[m];
      },
      //date
      D : function (date) {
          return date.getDate();
      ...
      DD: function(date){
          var d = dateUtil._formats.D(date);
          return dateUtil.leftZeroFill(d,2);
      },
      // weekday
      d : function (date) {
          return date.getDay();
      ...
      dd : function (date, language) {
          var d = dateUtil._formats.d(date);
          return dateUtil._dateLocale[language].weekdaysMin[d];
      },
      ddd : function (date, language) {
          var d = dateUtil._formats.d(date);
          return dateUtil._dateLocale[language].weekdaysShort[d];
      },
      dddd : function (date, language) {
          var d = dateUtil._formats.d(date);
          return dateUtil._dateLocale[language].weekdays[d];
      },
      // am pm
      a: function(date){
          if (date.getHours() > 12){
      ...
      hh: function(date){
          var h = dateUtil._formats.h(date);
          return dateUtil.leftZeroFill(h,2);
      },
      H: function(date){
          return date.getHours();
      },
      HH: function(date){
          return dateUtil.leftZeroFill(date.getHours(),2);
      },
      // minutes
      m: function(date){
      ...
      mm: function(date){
          return dateUtil.leftZeroFill(date.getMinutes(), 2);
      },
      //seconds
      s: function(date){
      ...
      ss: function(date){
          return dateUtil.leftZeroFill(date.getSeconds(),2);
      }
  },
  ...
  format: function(date, formatString, language){
      if (!date) return date;
      var array = formatString.match(dateUtil._formattingTokens), i, length,output='';
      var _date = dateUtil.getDateObj(date);
      if (!_date) return date;
      language = language || 'zh-CN';//u.core.getLanguages();
      for (i = 0, length = array.length; i < length; i++) {
          if (dateUtil._formats[array[i]]) {
              output += dateUtil._formats[array[i]](_date, language);
          } else {
              output += array[i];
          }
      }
  ...
  _addOrSubtract: function(date, period, value, isAdding){
      var times = date.getTime(),d = date.getDate(), m = date.getMonth(),_date = dateUtil.getDateObj(date);
      if (period === 'ms') {
  ...
  add: function(date,period,value){
      return dateUtil._addOrSubtract(date, period, value, 1);
  },
